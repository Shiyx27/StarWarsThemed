...
class CameraManager {
    ...
    checkEnvironment() {
        ...
        if (!isHTTPS) {
            console.warn('Camera may not work without HTTPS. Current protocol:', location.protocol);
            // Don't block completely - some browsers allow camera access on HTTP for localhost
        }
        
        if (!navigator.mediaDevices?.getUserMedia) {
            this.showError(
                'Browser Not Supported',
                'Your browser does not support camera access. Please try a modern browser.'
            );
            return false;
        }
    ...
    initializeElements() {
        ...
        if (!this.video) {
            console.warn('Video element not found, creating one...');
    ...
    this.outputCanvas.style.pointerEvents = 'auto'; // Allow mask interaction (mouse drag/wheel)
